...
# MODIFIED Rev 10: Adjusted defaults
...
def evaluate_ood(system, env, num_eval_steps=1000):
    ...
    with torch.no_grad():
        ...
        current_cls_state = system.ct

        for _ in range(num_eval_steps):
            task_input = env.step()
            a, op_idx, b, true_answer_c = task_input
            try:
                _ = system.task_head.predict(current_cls_state)
                predicted_answer = system.task_head.get_task_prediction()
...
def main(args):
    ...
    log_dir = args.log_dir or f"logs_p1_rev10_{time.strftime('%Y%m%d_%H%M%S')}" # Updated log name
    ...
    metrics = {}
    # MODIFIED Rev 10: Accumulator for recent task accuracy
    task_correct_buffer = []
    ...
    for step in pbar:
        ...
        try:
            ...
            # MODIFIED Rev 10: Accumulate task correct flag
            task_correct_buffer.append(metrics.get("TaskCorrect", 0))

            # Log metrics to buffer only periodically
            if step % args.log_save_interval == 0:
                # Calculate recent task accuracy
                if task_correct_buffer:
                    recent_acc = np.mean(task_correct_buffer)
                    metrics["TaskAccuracy_Recent"] = recent_acc
                    task_correct_buffer = [] # Reset buffer
                else:
                    metrics["TaskAccuracy_Recent"] = float('nan')

                # Fetch latest OOD metrics if available
                ...
                logger.log_metrics(metrics_to_log, step) # Log combined metrics
        ...
        if step % args.pbar_update_freq == 0 or step == args.total_steps - 1:
            ...
            pbar.set_postfix({
                ...
                "TaskAcc": f"{metrics.get('TaskAccuracy_Recent', float('nan')):.2f}", # MODIFIED Rev 10
                ...
                "LE_EMA": f"{metrics.get('lambda_max_EMA', float('nan')):.3f}",
        ...
        if args.eval_interval > 0 and step > 0 and step % args.eval_interval == 0:
             ...
             # Store OOD results in the main metrics dict to be picked up next log save
    ...
    if last_log_save_step < args.total_steps -1:
        ...
        logger.log_metrics(metrics, args.total_steps - 1)
        logger.save_log()
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train DPLD Model - Revision 10")
    # Model Params
    ...
    # Training Params
    parser.add_argument('--total-steps', type=int, default=50000, help='Total number of training steps') # MODIFIED Rev 10: Longer run
    parser.add_argument('--module-lr', type=float, default=5e-5, help='Learning rate for generic predictive modules')
    parser.add_argument('--taskhead-lr', type=float, default=3e-5, help='Learning rate for TaskHead module') # MODIFIED Rev 10: Increased
    ...
    parser.add_argument('--prediction-loss-weight', type=float, default=DEFAULT_PREDICTION_LOSS_WEIGHT, help='Weight for CLS prediction loss component') # Default changed in core.py
    parser.add_argument('--task-loss-weight', type=float, default=DEFAULT_TASK_LOSS_WEIGHT, help='Weight for Task prediction loss component (TaskHead only)') # Default changed in core.py
    # Dynamics Params
    parser.add_argument('--gamma-min', type=float, default=0.02, help='Min value for CLS decay rate gamma_t') # MODIFIED Rev 10: Increased slightly
    ...
    # Logging/Eval Params
    parser.add_argument('--log-dir', type=str, default=None, help='Directory for logs and checkpoints (default: logs_p1_rev10_YYMMDD_HHMMSS)')
    ...
    parser.add_argument('--eval-interval', type=int, default=2500, help='Evaluate OOD performance every N steps (-1 to disable)') # MODIFIED Rev 10: Adjusted
