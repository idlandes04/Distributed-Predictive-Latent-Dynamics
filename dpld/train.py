...
from core import DPLDSystem, DEFAULT_ACTION_STD, DEFAULT_PREDICTION_LOSS_WEIGHT, DEFAULT_TASK_LOSS_WEIGHT # Import new default
...
def main(args):
    ...
    log_dir = args.log_dir or f"logs_p1_rev8_{time.strftime('%Y%m%d_%H%M%S')}"
    ...
    system = DPLDSystem(
        ...
        task_loss_weight=args.task_loss_weight, # Pass task weight
    ...
    for step in pbar:
        ...
        try:
            ...
            logger.log_metrics(metrics, step) # Log metrics to buffer
        ...
        # --- Logging & Saving ---
        # Update progress bar description periodically (e.g., every second or few steps)
        if step % args.pbar_update_freq == 0 or step == args.total_steps - 1:
            ...
            pbar.set_postfix({
                ...
                "TaskSm": f"{metrics.get('TaskHead_Sm_log_task', float('nan')):.2f}", # Add Task Surprise
                ...
                "LE_EMA": f"{metrics.get('lambda_max_EMA', float('nan')):.2f}", # More precision
        ...
        if args.eval_interval > 0 and step > 0 and step % args.eval_interval == 0:
             ...
             # Log OOD results immediately after evaluation
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train DPLD Model - Revision 8")
    # Model Params
    ...
    # Training Params
    parser.add_argument('--total-steps', type=int, default=50000, help='Total number of training steps') # Increased
    parser.add_argument('--module-lr', type=float, default=5e-5, help='Learning rate for predictive modules')
    ...
    parser.add_argument('--action-std', type=float, default=DEFAULT_ACTION_STD, help='Standard deviation for module action sampling')
    ...
    parser.add_argument('--task-loss-weight', type=float, default=DEFAULT_TASK_LOSS_WEIGHT, help='Weight for Task prediction loss component (TaskHead only)') # Added
    # Dynamics Params
    ...
    # Stability Params
    parser.add_argument('--le-interval', type=int, default=2000, help='Interval for estimating Lyapunov exponent (-1 to disable)') # Re-enabled
    parser.add_argument('--stability-target', type=float, default=-0.01, help='Target value for lambda_max_EMA for Meta-Model reward') # Keep near 0
    # Environment Params
    parser.add_argument('--max-val', type=int, default=100, help='Maximum value for numbers in arithmetic task')
    # Logging/Eval Params
    parser.add_argument('--log-dir', type=str, default=None, help='Directory for logs and checkpoints (default: logs_p1_rev8_YYMMDD_HHMMSS)')
    parser.add_argument('--pbar-update-freq', type=int, default=10, help='Update progress bar postfix every N steps')
    parser.add_argument('--log-save-interval', type=int, default=500, help='Save metrics log file every N steps') # Save periodically
    parser.add_argument('--eval-interval', type=int, default=2000, help='Evaluate OOD performance every N steps (-1 to disable)') # Re-enabled
